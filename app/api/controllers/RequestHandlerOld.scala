...
object RequestHandlerOld {
  ...
  case class RequestHandlerBuilder[InputRaw <: RawData, Input, Output] private[RequestHandlerOld](
    ...
    private object Delegate extends RequestHandlerOld[InputRaw] with Logging with RequestContextImplicits {
      ...
      def handleRequest(
        ...
        val result =
          if (simulateRequestCannotBeFulfilled) {
            EitherT.fromEither[Future](Left(ErrorWrapper(ctx.correlationId, RuleRequestCannotBeFulfilledError)))
          } else {
            for {
      ...
      private def simulateRequestCannotBeFulfilled(implicit request: UserRequest[_], appConfig: AppConfig) =
